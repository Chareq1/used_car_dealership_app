...
public partial class VehicleAddView : UserControl
{
    // Digits
    ...
    private void OnlyDecimalDigitsKeyDown(object? sender, KeyEventArgs e)
    {
        if (!Digits.Contains(e.Key) && e.Key != Key.OemComma && e.Key != Key.Back)
        {
            e.Handled = true;
        }
    }
    
    private void OnlyBrandAndModelKeyDown(object? sender, KeyEventArgs e)
    {
        if (!Letters.Contains(e.Key) && !Digits.Contains(e.Key) && e.Key != Key.OemMinus && e.Key != Key.OemPeriod && e.Key != Key.Space && e.Key != Key.Back)
        {
            e.Handled = true;
        }
    }
    
    private void OnlyVINKeyDown(object? sender, KeyEventArgs e)
    {
